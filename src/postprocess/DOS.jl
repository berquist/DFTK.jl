...
function NOS(ε, basis, eigenvalues; smearing=basis.model.smearing,
             temperature=basis.model.temperature, spins=spin_components(basis.model))
    N = zero(ε)
    if (temperature == 0) || smearing isa Smearing.None
        error("NOS only supports finite temperature")
    end
    @assert all(σ -> σ in spin_components(basis.model), spins)
...
function LDOS(ε, basis, eigenvalues, ψ; smearing=basis.model.smearing,
    ...
    if length(spins) == 1 && spins[0] == :up
        return (ldostot.real + ldosspin.real) / 2
    elseif length(spins) == 1 && spins[0] == :down
        return (ldostot.real - ldosspin.real) / 2
    else
        return ldostot.real
    end
